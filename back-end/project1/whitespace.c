...
void bldident() 
{
  ...
  do // read NO OLD COMMENT NOW BAD NEW COMMENT 
  { 
    if (k < idbuffsize) 
    {
      idbuff[ k] = ch;
...
// hello new whitespace and new comment in the new whitespace 
...
void scannum() 
{
  ...
  if ( ch == '.') 
  {
    ...
    else if( inSeparators( lookahead))  { // yeah that's right this is a new comment
      numdigs = 0;
      decimals = 0;
      sym = real_number;
      nextchar();
...
void writesym() 
{
  ...
  switch( sym) 
  {
    ...
    case real_number:
      ...
      break;
    default: break;
  } // end switch AND EDIT THE COMMENT
...
int main( int argc, char **argv) 
{
   ...
   return(0);
} // end THE PROGRAMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM
